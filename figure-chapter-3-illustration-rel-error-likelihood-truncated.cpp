...
#pragma omp for
	...
	if (!std::signbit(rho)) {
	  ...
	  double out_small_t =
	    solver.numerical_likelihood_first_order_small_t_ax_bx_ay_by_type_41_truncated(&raw_input.vector,
										small_t,
										dx_likelihood);
	  ...
	  std::vector<std::vector<double>> small_t_likelihood_matrix =
	    std::vector<std::vector<double>> (M, std::vector<double>(M,0.0));
	  std::vector<std::vector<double>> analytic_likelihood_matrix =
	    std::vector<std::vector<double>> (M, std::vector<double>(M,0.0));
	  ...
	  for (unsigned jj=0; jj<M; ++jj) {
	    for (unsigned ii=0; ii<M; ++ii) {
	      ...
	      printf("x=%g, y=%g ", xs[jj], ys[ii]);
	      double out_small_t =
		solver.numerical_likelihood_first_order_small_t_ax_bx_ay_by_type_41(&raw_input.vector,
										    small_t,
										    dx_likelihood);
	      small_t_likelihood_matrix[jj][ii] = out_small_t;

	      analytic_likelihood_matrix[jj][ii] =
	  	solver.analytic_likelihood(&raw_input.vector, 1000);

	      double rel_diff =
	  	std::abs(analytic_likelihood_matrix[jj][ii]-
	  		 out_small_t)/
	  	analytic_likelihood_matrix[jj][ii];
	  ...
	  for (std::vector<std::vector<double>>::iterator it=small_t_likelihood_matrix.begin();
	       it != small_t_likelihood_matrix.end(); ++it) {

	    if (it != small_t_likelihood_matrix.end()-1) {
	      for (double sol : *it) {
	  	printf("%g, ", sol);
	      }
	      printf("\n");
	  ...
	  for (std::vector<std::vector<double>>::iterator it=analytic_likelihood_matrix.begin();
	       it != analytic_likelihood_matrix.end(); ++it) {

	    if (it != analytic_likelihood_matrix.end()-1) {
	      for (double sol : *it) {
	  	printf("%g, ", sol);
	      }
	      printf("\n");
	  ...
	  printf("pdf(\"./src/kernel-expansion/documentation/chapter-3/chapter-3-figure-illustration-rel-error-likelihood-truncated.pdf\", 6,6)\n");
